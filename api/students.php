...
if ($method === 'POST') {
    ...
    $body = getBody();
    $last=$body['last_name']??''; $first=$body['first_name']??''; $mid=$body['middle_name']??'';
    $age=$body['age']??''; $sex=$body['sex']??''; $usn=$body['usn']??'';
    $lrn=$body['lrn']??''; $sec=$body['section']??''; $email=$body['guardian_email']??'';
...
if (isset($_GET['all'])) {
    ...
    ob_end_clean(); respond(['success'=>true,'students'=>$students]);
}
